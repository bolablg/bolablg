# Bolaji BALOGOUN

**Head of Data & AI · I turn data into decisions, products, and margin.**

I build and lead teams that engineer end-to-end data systems and AI applications: platform, products, governance, FinOps, and the people who run them.

<p>
  <a href="https://www.bolablg.com"><img src="https://img.shields.io/badge/Portfolio-bolablg.com-2419ff?style=flat-square&logo=googlechrome&logoColor=white" alt="Portfolio" /></a>
  <a href="https://chat.bolablg.com"><img src="https://img.shields.io/badge/iBola-ask_my_AI_assistant-111111?style=flat-square&logo=googlegemini&logoColor=white" alt="iBola AI assistant" /></a>
  <a href="https://blog.bolablg.com"><img src="https://img.shields.io/badge/Blog-blog.bolablg.com-FF6719?style=flat-square&logo=substack&logoColor=white" alt="Blog" /></a>
  <a href="https://linkedin.com/in/bolablg"><img src="https://img.shields.io/badge/LinkedIn-in/bolablg-0A66C2?style=flat-square" alt="LinkedIn" /></a>
  <a href="https://x.com/bolablg"><img src="https://img.shields.io/badge/X-@bolablg-111111?style=flat-square&logo=x&logoColor=white" alt="X" /></a>
</p>

> **Skip the reading. Ask iBola.**
> My portfolio ships with its own AI assistant, an agentic RAG system grounded in [bolablg.com](https://www.bolablg.com) and scoped to my professional life. Ask it anything about my work: **[chat.bolablg.com](https://chat.bolablg.com)**

---

### What I do

At [Gozem](https://gozem.co), the super app for mobility, commerce, and fintech in Africa, I was the group's first data hire. Through July 2026 I owned the data function end to end (platform, products, team, governance, FinOps) and co-led the AI strategy. I scaled the team from 0 to 14+ across Sub-Saharan Africa and shipped the Gozem Data Hub, a central platform serving over 650 people (employees, field agents, and partners) with data products, AI tools, and automation, while cutting Google Cloud spend by 43% (from $10K+ to under $6K/month).

I also consult. With the **UN International Trade Centre**, I shaped data strategy for West African startups.

---

### Track record

**Leadership & scale**
- Built Gozem's data team from scratch: 0 to 14+ people across Sub-Saharan Africa
- Designed and shipped the Gozem Data Hub, serving over 650 employees, field agents, and partners
- Cut GCP spend by 43%, from $10K+ to under $6K/month, while the platform kept scaling

**Consulting & research**
- Data strategy consulting for the **UN International Trade Centre** (West African startups)
- Co-published multilingual African language datasets: [Zenodo](https://zenodo.org/record/4266935) · [AfricaNLP, EACL 2021](https://arxiv.org/abs/2104.02516)
- Zindi AI4D Dataset Challenge winner with [Takwimu Lab](https://github.com/Takwimu-Lab)

**Community**
- Founding Vice-Chair, [iSheero](https://isheero.com): growing AI and data science talent in Francophone Africa
- Co-founder, [Takwimu Lab](https://github.com/Takwimu-Lab): NLP research for African languages

---

### Live products & open source

| Project | What it is |
|---------|------------|
| [**iBola**](https://chat.bolablg.com) · live | Agentic RAG assistant answering questions about my professional background. LangGraph orchestration, hybrid retrieval (semantic + BM25), guardrails, Langfuse observability, Gemini |
| [**Apps & tools**](https://app.bolablg.com) · live | Self-built apps and productivity tools at app.bolablg.com |
| [**uDownloader**](https://github.com/bolablg/uDownloader) | Fast async video downloader (CLI + desktop GUI) for YouTube, Instagram, TikTok, X, Vimeo. On [PyPI](https://pypi.org/project/udownloader/): `pip install udownloader` |
| [**GCP Dataform Modeling Lab**](https://github.com/bolablg/gcp-dataform-modeling-lab) | Production-tested Dataform framework for BigQuery: incremental logic, schema evolution, cost-saving assertions, modular pipelines |
| [**Salaire BÉNIN**](https://github.com/bolablg/Traitement-de-salaire-BJ) | Net/gross salary calculator for Benin under the 2025 finance law, with full contribution and tax breakdown |

---

### Stack

```text
AI & LLM         LangGraph · RAG · Vector DBs · LLM APIs · Agentic workflows · MCP
Cloud & Data     BigQuery · Dataform · Vertex AI · Dataflow · Cloud Run · Pub/Sub · GCS
Visualization    Looker · Metabase · Plotly · Analytics web apps
Languages & DB   Python · SQL · R · JavaScript · Bash · PostgreSQL · MongoDB · ClickHouse
DevOps           Airflow · Docker · Git · GitHub Actions · AWS EC2
```

---

### GitHub activity

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://streak-stats.demolab.com?user=bolablg&hide_border=true&background=00000000&ring=6c63ff&fire=6c63ff&currStreakLabel=6c63ff&currStreakNum=fafaf7&sideNums=fafaf7&sideLabels=fafaf7&dates=9a9a96" />
    <img height="170" src="https://streak-stats.demolab.com?user=bolablg&hide_border=true&background=00000000&ring=2419ff&fire=2419ff&currStreakLabel=2419ff&currStreakNum=111111&sideNums=111111&sideLabels=111111&dates=767672" alt="Bolaji Balogoun contribution streak" />
  </picture>
</p>

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://github-readme-activity-graph.vercel.app/graph?username=bolablg&hide_border=true&hide_title=true&bg_color=00000000&color=fafaf7&line=6c63ff&point=fafaf7&area=true&area_color=6c63ff" />
    <img src="https://github-readme-activity-graph.vercel.app/graph?username=bolablg&hide_border=true&hide_title=true&bg_color=00000000&color=111111&line=2419ff&point=111111&area=true&area_color=2419ff" alt="Bolaji Balogoun contribution graph" />
  </picture>
</p>

---

<p align="center">
  <a href="https://www.bolablg.com">bolablg.com</a> · <a href="https://chat.bolablg.com">Ask iBola</a> · <a href="https://blog.bolablg.com">Blog</a> · <a href="https://linkedin.com/in/bolablg">LinkedIn</a> · <a href="mailto:hello@bolablg.com">hello@bolablg.com</a>
</p>
